...
class CustomerActivityStatementWizard(models.TransientModel):
    ...
    aging_type = fields.Selection(
        [('days', 'Age by Days'),
         ('months', 'Age by Months')], string='Aging Method', default='months', required=True)

    def _default_date_start(self):
        return fields.Date.to_string((date.today() - timedelta(months=1)).replace(day=1))

    def _default_date_end(self):
        return fields.Date.to_string(date.today().replace(day=1) - timedelta(days=1))
