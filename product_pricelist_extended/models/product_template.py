...
class ProductTemplate(models.Model):
    ...
    assortment_id = fields.Many2one(
        ...
        domain=[("is_assortment", "=", True), ("show_in_filters", "=", True)],
        store=False,
        search="_search_assortment_filter",
        help="Technical field to filter by assortment membership",
    )
        ...
        for ir_filter in self.env["ir.filters"].browse(value):
            if not domain:
                ...
                domain = expression.OR(
                    [domain, getattr(ir_filter, f"_get_eval_domain_{model}")()]
                )
        return domain
