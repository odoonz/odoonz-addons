            ...
            for rule in items:
                ...
                if is_product_template:
                    ...
                    if rule.product_tmpl_ids and (
                        product.id not in rule.product_tmpl_ids.ids
                    ):
                        continue
                    if (
                        rule.product_ids
                        and product.product_variant_count == 1
                        and (product.product_variant_id.id not in rule.product_ids.ids)
                    ):
                        continue
                    ...
                    if (
                        ...
                        and product.product_variant_count == 1
                        and (
                            product.id not in rule.price_categ_id.product_tmpl_ids.ids
                            and product.product_variant_id.id
                            not in rule.price_categ_id.product_ids.ids
                        )
                    ):
                        continue
                else:
                    ...
                    if rule.price_categ_id and (
                        ...
                        not in rule.price_categ_id.product_tmpl_ids.ids
                        and product.id not in rule.price_categ_id.product_ids.ids
                    ):
                        continue
