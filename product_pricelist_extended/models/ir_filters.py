...
class IrFilters(models.Model):
    ...
    def _get_eval_domain_template(self):
        ...
        if self.model_id == "product.category":
            ...
            return [("id", "in", self.env['product.product'].search(orig_res).product_tmpl_id.ids)]
        else:
            return orig_res
        return new_res
    ...
    def _get_pricelist_domain(self):
        self.ensure_one()
        return [('id', 'in', self.env['product.pricelist.assortment.item'].search([('assortment_filter_id', '=', self.id)]).pricelist_id.ids)]
    ...
    def _compute_record_count(self):
        for record in self:
            ...
            record.pricelist_record_count = self.env['product.pricelist'].search_count(domain)
            ...
            record.categ_record_count = self.env['product.category'].search_count(domain)
    ...
    def _get_action_domain(self, action_id=None):
        ...
        domain = expression.AND([['|', ("is_assortment", "=", False), ("show_in_filters", "=", True)], domain])
        return domain
