    ...
    def _compute_product_id(self):
        ...
        for bom_line in self:
            if bom_line._context.get("product_id"):
                parent_product = Product.browse([bom_line._context.get("product_id")])
                ...
                if bom_line.required_value_ids:
                    for value in bom_line.required_value_ids:
                        search_domain.append(("attribute_value_ids", "in", [value.id]))
                ...
                if common_attrs:
                    ...
                    for value_id in value_ids:
                        search_domain.append(("attribute_value_ids", "in", [value_id]))
                ...
                product = bom_line.variant_id
            bom_line.product_id = self.env["xform.substitution.map"]._get_substitute(
                product
            )
