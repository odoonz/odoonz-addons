...
class MrpBom(models.Model):
    ...
    def _explode_scale_weight_kg(bom_line, line_fields):
        if bom_line.product_uom_id != bom_line.env.ref(
            ...
            _logger.error(_('Scale weight only works for '
        ...
        parent_weight = (
            ...
            ) * line_fields['product'].weight or 1.0
        )
        qty = (parent_weight * line_fields['original_qty'] *
               bom_line.product_qty)
        rounding = bom_line.product_uom_id.rounding
        line_fields.update(qty=float_round(
