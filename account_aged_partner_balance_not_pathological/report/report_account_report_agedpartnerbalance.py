...
class ReportAccountReportAgedpartnerbalance(models.AbstractModel):
    ...
    def _get_partner_move_lines(
        ...
        res = []
        total = [0] * 7  # removing pointless loop lower
        partner_clause = ""
        cr = self.env.cr
        ...
        if ctx.get("partner_categories"):
            ...
            partner_ids = (
                ...
                .search([("category_id", "in", ctx["partner_categories"].ids)])
                .ids
            )
        ...
        query = (
            ...
            FROM account_move_line AS l
                ...
                left join account_partial_reconcile apr_credit
                    ON l.id = apr_credit.credit_move_id
                left join account_partial_reconcile apr_debit
                    ON l.id = apr_debit.debit_move_id,
        ...
        # history[1] = {'<partner_id>': <partner_debit-credit>}
        history = []
        for i in range(5):
        ...
        for partner in partners:
            ...
            if partner["partner_id"]:
                # browse the partner name and trust field in sudo, as we may not
                # have full access to the record (but still have to see it in report)
                browsed_partner = (
                    self.env["res.partner"].sudo().browse(partner["partner_id"])
                )
                values["name"] = (
