...
class TestProductCombinationExclude(TransactionCase):
    ...
    def setUp(self):
        ...
        self.matrix1 = self.browse_ref(
            'product_attribute_exclusion.exclude_matrix_test_1')

        #  This matrix says we cannot have Green T-Shirts
        #  in large size
        self.matrix2 = self.browse_ref(
            'product_attribute_exclusion.exclude_matrix_test_2')

        #  all 3 products have the same attribute values, but different
        #  exclusion matrices
        self.product1 = self.browse_ref(
            'product_attribute_exclusion.product_exclude_test_1')
        self.product2 = self.browse_ref(
            'product_attribute_exclusion.product_exclude_test_2')
        self.product3 = self.browse_ref(
            'product_attribute_exclusion.product_exclude_test_3')
        self.product_obj = self.env['product.product']

        self.green = self.browse_ref(
            'product_attribute_exclusion.product_attribute_value_7')
        self.purple = self.browse_ref(
            'product_attribute_exclusion.product_attribute_value_9')
        self.medium = self.browse_ref(
            'product_attribute_exclusion.product_attribute_value_2')
        self.large = self.browse_ref(
            'product_attribute_exclusion.product_attribute_value_3')

        self.attr_line_2 = self.browse_ref(
            'product_attribute_exclusion.product_attribute_line_2')

        self.product_attr2 = self.browse_ref(
            'product_attribute_exclusion.product_attribute_2'
        )
